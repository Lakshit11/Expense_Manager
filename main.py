...
class SignUpScreen(Screen):
    def adduser(self, uname, p1, p2):
        ...
        if p1 == p2:
            if uname in users:
                self.ids.same.text = "User Already Exists!! Please try different Username"
...
class ViewScreen(Screen):
    ...
    def viewdata(self):
        ...
        data = df1.sort_values(by = 'Date', ascending = False)
        var = data.to_string(index=True)
        self.ids.quote.text = "Your total expense on date is : \n" + "  " + str(var)
    
    def viewgraph(self):
        view = ModalView(size_hint=(0.90, 0.50))
        df = pd.read_csv(CSV_FILE)
...
class CommentScreen(Screen):
    ...
    def viewcomments(self,date):
        ...
        if date in list(df['Date']):
            df1 = df[df['Date'] == date][['Comment','Amount']]
